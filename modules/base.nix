...

{
  ...
  environment.systemPackages = with pkgs; [
    ...
    # `fleet` / `fleet update` — status + deploy TUI (bases-themed). It excludes
    # the laptops by default (excludeHosts in pkgs/fleet.nix) — they deploy
    # locally and are usually asleep — so nothing to pass here.
    (callPackage ../pkgs/fleet.nix { })
