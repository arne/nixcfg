...

{
  ...
  services.greetd = {
    ...
    settings = {
      ...
      #
      # NB: the kernel-7.0 "fbcon now renders on this panel" theory was tried
      # (tuigreet console greeter, 2026-06-13) and reverted 2026-06-16 — the
      # Studio Display stayed blank at the greeter, so login was impossible.
      # The framebuffer binds at 5K, but the panel shows nothing for the bare
      # console stream; only niri drives it. A login gate needs a *Wayland*
      # greeter, not a console one.
      #
